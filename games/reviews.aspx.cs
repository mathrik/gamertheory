...
public partial class games_reviews : System.Web.UI.Page {
	...
	protected void Page_Load(object sender, EventArgs e) {
        ...
        if ((Convert.ToDouble(totalstories) / Convert.ToDouble(numperpage)) > Convert.ToDouble(page)) {
            litRightPaging.Text = "<a href='/games/reviews.aspx?p=" + (page + 1).ToString() + "'>Older Stories&gt;&gt;</a>";
        }
        if (page > 1 && (Convert.ToDouble(totalstories) / Convert.ToDouble(numperpage)) > 1) {
            litLeftPaging.Text = "<a href='/games/reviews.aspx?p=" + (page - 1).ToString() + "'>&lt;&lt;Newer Stories</a>";
        }
